...
export function generateMetadata({ params }: Props): Metadata {
  ...
  return buildMetadata({
    locale,
    title: locale === "tr" ? "AI Aracini Gonder" : "Submit Your AI Tool",
    description:
      locale === "tr"
        ? "AI aracini directory'ye eklemek icin formu doldur."
  ...
  return (
    <section className="mx-auto max-w-2xl rounded-2xl border border-slate-800 bg-slate-900 p-6">
      <h1 className="text-3xl font-bold text-slate-100">
        {locale === "tr" ? "AI aracini gonder" : "Submit your AI tool"}
      </h1>
      <p className="mt-2 text-slate-300">
        {locale === "tr"
          ? "Form moderasyon sonrasi yayina alinacak."
      ...
      <form action="/api/submit-tool" method="post" className="mt-6 space-y-4">
        <input
          name="name"
          placeholder={locale === "tr" ? "Arac adi" : "Tool name"}
          className="w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2"
          required
        />
        <textarea
          name="description"
          placeholder={locale === "tr" ? "Aciklama" : "Description"}
